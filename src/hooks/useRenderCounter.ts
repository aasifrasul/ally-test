...
export const useRenderCounter = (componentName: string = 'Component', verbose: boolean = true): ReturnType => {
	...
	useEffect(() => {
		...
		if (verbose) {
			...
			if (renderCount.current > 1) {
				console.log(`Time since last render: ${getTimeSinceLastRender().toFixed(2)}ms`);
				console.log(`Possible reason: ${renderReasons.current[renderReasons.current.length - 1] || 'Unknown'}`);
			}
