...
export function useSearchParams() {
	...
	const setSearchParams = useCallback((newParamsOrUpdater: URLSearchParams | ((prev: URLSearchParams) => URLSearchParams)) => {
	...
	function handlePopState(event: PopStateEvent) {
		...
		const newParams = new URLSearchParams(event.state?.searchParams || window.location.search);
