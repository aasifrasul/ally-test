...
describe('FlipTheCard Component', () => {
	...
	test('initial state should have no open cards', () => {
		...
		expect(
			screen
				.queryAllByTestId(/card-\d+/)
				.filter((el) => el.classList.contains('card-open')),
		).toHaveLength(0);
	});
	...
	test('clicking on two non-matching cards should close them after delay', async () => {
		...
		await act(async () => {
			const rafCallback = jest.requireMock('../../utils/typeChecking')
				.safelyExecuteFunction.mock.calls[0][0];
			rafCallback();
		});
	...
	test('completing the game should call onGameComplete callback', async () => {
		...
		render(<FlipTheCard mockCards={mockCards} onGameComplete={mockOnGameComplete} />);
	...
	test('should not allow clicking on already open cards', () => {
		...
		// Try to click cards
		fireEvent.click(
			screen.queryByTestId('card-back-0') ||
				screen.queryByTestId('card-image-0') ||
